...
function book(id){if(menuDialog.open)menuDialog.close();interest.value=id;$('#booking').scrollIntoView({behavior:reduced?'instant':'smooth'});interest.focus({preventScroll:true});}document.addEventListener('click',e=>{const booking=e.target.closest('[data-book]');if(booking)book(booking.dataset.book);const detail=e.target.closest('[data-detail]');if(detail){menuDialog.showModal();$('#detail-'+detail.dataset.detail).scrollIntoView({block:'center'});}});$('#menu-open').onclick=()=>menuDialog.showModal();$('#consult').onclick=()=>book('consult');document.querySelectorAll('dialog').forEach(dialog=>{dialog.querySelector('.close').onclick=()=>dialog.close();dialog.addEventListener('click',e=>{if(e.target===dialog){const r=dialog.getBoundingClientRect();if(e.clientX<r.left||e.clientX>r.right||e.clientY<r.top||e.clientY>r.bottom)dialog.close();}});});$('#booking-open').onclick=()=>{$('#chosen').textContent='Bạn đang quan tâm: '+interest.options[interest.selectedIndex].text;bookingDialog.showModal();};$('.close-booking').onclick=()=>bookingDialog.close();
...
if(therapy){
  ...
  function updateTherapy(){
    ...
    therapy.style.setProperty('--therapy-keywords-opacity',keywordsOpacity.toFixed(4));
    therapy.style.setProperty('--therapy-keywords-y',`${(16*(1-keywordsOpacity)).toFixed(1)}px`);
